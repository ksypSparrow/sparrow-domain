...
@Suite("The 1.0.0 surface")
struct PublicSurfaceTests {
    /// A saved search is a filter **and** a sort. Storing one without the
    /// other would be half a saved search, which is why both are `Codable`.
    @Test("A filter and its sort round-trip together")
    func filterAndSortEncodeTogether() throws {
        struct SavedSearch: Codable, Hashable {
            var filter: NoteFilter
            var sort: NoteSort
        }
        let original = SavedSearch(
            filter: NoteFilter(text: "heron", kinds: [.sketch], isPinned: true),
            sort: NoteSort(field: .observed, order: .ascending)
        )

        let data = try JSONEncoder().encode(original)
        #expect(try JSONDecoder().decode(SavedSearch.self, from: data) == original)
    }

    /// ⚠️ The reason `NoteEdit` and `NotebookEdit` are **not** `Codable`.
    ///
    /// A doubly-optional field does not survive JSON: `.some(nil)` encodes as
    /// `null` and decodes back as `nil`, so *clear this field* would silently
    /// become *leave it alone*. This test pins the behaviour that made the
    /// decision, so nobody adds the conformance later thinking it is free.
    @Test("A double optional does not survive JSON, which is why edits are not Codable")
    func doubleOptionalsDoNotRoundTrip() throws {
        struct Probe: Codable, Equatable {
            var value: Date??
        }
        let clearIt = Probe(value: .some(nil))

        let data = try JSONEncoder().encode(clearIt)
        let restored = try JSONDecoder().decode(Probe.self, from: data)

        // `.some(nil)` came back as `nil` — a different instruction entirely.
        #expect(restored != clearIt)
        #expect(restored.value == nil)
    }

    /// Every identifier is client-generated, which is what lets two offline
    /// devices create records without colliding — and what NFR-1.3 rests on.
    @Test("Identifiers are generated by the app, never by a database")
    func identifiersAreClientGenerated() {
        #expect(NoteID() != NoteID())
        #expect(NotebookID() != NotebookID())
        // A tag's identity is its text, so equal labels *must* collide.
        #expect(TagID(normalizing: "Wetlands") == TagID(normalizing: "wetlands"))
    }
}
